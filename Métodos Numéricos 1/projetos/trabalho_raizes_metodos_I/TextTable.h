...
public:
  ...
  TextTable(string horizontal, string vertical, string corner)
  ...
  string vertical() const { return _vertical; }

  string horizontal() const { return _horizontal; }
...
private:
  ...
  static std::string repeat(unsigned times, string c) {
    std::string result;
    for (; times > 0; --times)
      result += c;
